...
type OutboxMessage struct {
	...
	EventType string
	OrderId   string
	// Тут продюсит ток Id, однако я хз. Парсинг его с payload'а не по кайфу как-та...
	// Payload   []byte
	CreatedAt time.Time
}
...
func (op *OutboxProcessor) processOutbox(ctx context.Context) {
	...
	rows, err := tx.Query(ctx,
		`SELECT id, topic, event_type, order_id, created_at 
		FROM outbox 
	...
	for rows.Next() {
		...
		if err := rows.Scan(&msg.ID, &msg.Topic, &msg.EventType, &msg.OrderId, &msg.CreatedAt); err != nil {
	...
	for _, msg := range messages {
		err = op.prod.Produce(msg.Topic, msg.OrderId)
		if err != nil {
